...
# Create necessary directories
mkdir -p config certs logs data
...
test_config() {
    ...
    if [ -f config/config.yaml ] && [ -f config/auth-config.yaml ]; then
        ...
        echo -e "\${RED}❌ FAIL\${NC}"
        echo "Missing configuration files. Run setup.sh first."
        return 1
    fi
}
...
test_ssl() {
    ...
    if [ -f certs/server.crt ] && [ -f certs/server.key ]; then
        ...
        echo -e "\${RED}❌ FAIL\${NC}"
        echo "Missing SSL certificates. Run ./generate-self-signed.sh"
        return 1
    fi
}
...
export async function GET(
  ...
  try {
    ...
    // Check if file exists in our templates
    if (!deploymentFiles[fileName as keyof typeof deploymentFiles]) {
      return NextResponse.json({ error: 'File not found' }, { status: 404 });
    }

    const fileContent = deploymentFiles[fileName as keyof typeof deploymentFiles];
